...
import pincodeListModule from "./pincode-list.vue";
...
export default {
	...
	components: { "header-list": headerList,'pincode-list':pincodeListModule },
	methods: {
		deleteRecord(id) {
			let _vm = this;
			this.$dialog
		...
		getQueryString() {
			let queryString =
				"?type=deliveryboy&page=" + this.paginate.current_page;
			queryString +=
				this.searchText.length > 0 ? "&search=" + this.searchText : "";
			return queryString;
		},
		getPincodeByDeliveryBoy(id){
			let _vm = this;
			this.$store.state.isLoaderShow = true;
			_vm.pincodeList=[];
			this.axios
				.get("/pincode-by-delieveryboy/" + id)
				.then(function(response) {
					_vm.$store.state.isLoaderShow = false;
					_vm.pincodeList=response.data.data;
				})
				.catch(function(error) {
					console.log(error);
				});
